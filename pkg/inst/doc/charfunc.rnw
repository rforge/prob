...
are not known in closed form. I would be interested in any reference
to the contrary.
...
\[
\phi_{X}(t)=\frac{\exp(\frac{i\delta t}{1-2it})}{(1-2it)^{p/2}}.\]
...
_{2}F_{1}(a,\, b;\, c;\, z)=\sum_{n=0}^{\infty}\frac{(a)_{n}(b)_{n}}{(c)_{n}}\,\frac{z^{n}}{n!},\]
 where $(a)_{n}$ is the rising factorial defined as above in Section
\ref{sub:Beta-distribution}. See \cite{GaussHyper} in the References
for details concerning $_{2}F_{1}$. We calculate it by means of the
\texttt{hypergeo} function in the \Rpackage{hypergeo} package.
...
See \cite{lnormCF} in the References.
...
Let $p$ denote the \texttt{df} parameter. The characteristic function
is given by \[
...
which has the default value of \texttt{kmax} = 10. 
...
\begin{itemize}
  ...
  \item Base packages: base, datasets, graphics, grDevices, methods,
    stats, tcltk, utils
  \item Other packages: svMisc~0.9-45, svSocket~0.9-42
